...
/* COMEÇAR A COLOCAR OS PRAGMA! */
// 
// pragmas comentados são para testar depois e fazer as comparações para o relatório.
// 
...
int recupera(typeProb malha[N][N], int linha, int coluna, int gama, int d) {
    ...
    if(malha[linha][coluna].inf > 1) {
        ...
        if( ((d*soma)/8 >= 0.1) || (malha[linha][coluna].inf > 15) )
            return S;
...
void vacinaPulsadaLocalidade(typeProb malha[N][N]) {
    ...
    //#pragma omp parallel for private(i,j,seed) shared(aleat) 
    for ( j = 0; j < MAX ; ++j ) {
        seed = 25234 + 17*j;
        srand(seed);

        for(i = 0 ; i < (NRANDS/MAX) ; ++i) 
            aleat[i+((NRANDS/MAX)*j)] = rand(); /*rand_r(&seed);*/

    }

    size = N-2;

    //#pragma omp parallel for private(i,j,idx,lin,col) shared(aleat, malha) 
    for (i = 1; i < N ; i += PSEUDO) 
        for (j = 1; j < N ; j += PSEUDO)  {
